...
## XXX uncomment for debug (if you suspect that notification format has been changed)
# printf "s: '%s'\nb: '%s'\n\n" "$summary" "$body" >> ~/tmp_tls_log.txt

# toggle behavior of showing notifications (ALL/ONLY LISTED)
# 1 - show all, otherwise suppress notifications of not listed channels
TLC_SHOW_ALL=0

# sed get only channel name inside surrounding: <b> </b>
twitch_channel=$(echo "$body" | sed -n "s/^.*>\(.*\)<.*$/\1/p")
# remove all unprintable including spaces (just in case)
...
if grep -i "$twitch_channel" "$auto_file"; then
...
if [ -n "$TLC_SHOW_ALL" ] && [ "$TLC_SHOW_ALL" != 1 ]; then
    ...
    if ! grep -i "$twitch_channel" "$notify_file"; then
